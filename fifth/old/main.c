...
char* strSpace[40]; //empty array to receive both string separated by a space
char* strComa[40]; //empty array to receive both string separated by a coma
char* strComaRep[40]; //empty array to receive both string separated by a coma again
char* strSemicol[40]; //empty array to receive both string separated by a semi column
char action[40]; //after StrTokenizer will contain the action
char form[40]; //after StrTokenizer will contain the shape
int x, y, z; //x,y,z coordinates after StrTokenizer

char buffer[256];//Buffer
int num; // num may be useful later to know the size of the array
...
int StrTokenizer(char myString[], char myDelim[], char *myArr[40]){ // Function to decompose in tokens the string received by the Board from the user
  
  int i = 0;
  //splitting string into tokens according to delimiter
  ...
  //get size of array but no used in the end
...
int ReadUserInput(){ //function to read what the user input from PC to Board
  ...
  // Init variables
	...
	{
		char recBuffer[256];//Buffer inside
		uint8_t byte;
        // Receive serial data example
		/* Transmit one byte with 100 ms timeout */
		oRecvStatus = HAL_UART_Receive( &hUART2, recBuffer, 1, 100);

		if( oRecvStatus == HAL_OK )
		{

			byte = recBuffer[0];
			sprintf(recBuffer, "%c", byte);

			buffer[i] = byte;
		
			if(buffer[i] != '\n'){
				HAL_UART_Transmit( &hUART2, (uint8_t*)recBuffer, strlen(recBuffer), 0xFFFF);
				i++;
            } else {
...
void main(void)
{
	...
	{
		msg = "Welcome to Nucleo F401RE, tell me what to do !\r\n";
	...
	while(strncmp(buffer, "QUIT", 4) != 0) // main loop
	{
		//setting variables to get out of loops
		ReadOut = 1; 
		OutLCD = 1;
		CustomOut = 1;

		// line varibable incremented for the shape
		line = 1;
		...
		if(strncmp(buffer, "4", 1) == 0){
			HAL_UART_Transmit( &hUART2, (uint8_t*)"Displaying file on LCD...\r\n", strlen("Displaying file on LCD...\r\n"), 0xFFFF);
			...
			while(OutLCD) {
				...
				if(strncmp(buffer, "OUT", 3) != 0){
					...
					BSP_LCD_SetTextColor( LCD_COLOR_RED );
					BSP_LCD_DisplayStringAtLine( line, (uint8_t *)buffer);
					line++;
				} else {
					OutLCD = 0;
				}
			...
			} // while() reading joystick
		} else if (strncmp(buffer, "5", 1) == 0) {
			HAL_UART_Transmit( &hUART2, (uint8_t*)"Drawing custom command...\r\n", strlen("Drawing custom command...\r\n"), 0xFFFF);
			BSP_LCD_Clear( LCD_COLOR_WHITE );
			while(CustomOut){
				ReadUserInput();
				
				//splitting with space
				StrTokenizer(buffer, " ", strSpace);
				
				//splitting with semicolumns
				StrTokenizer(strSpace[0], ":", strSemicol);

				//copying data to characters
				strcpy(action, strSemicol[0]);
				strcpy(form, strSemicol[1]);

				//splitting the comas
				StrTokenizer(strSpace[1], ",", strComa);
				StrTokenizer(strComa[1], ",", strComaRep);
				...
				if(strncmp(action, "DRAW", 4) == 0){
					BSP_LCD_Clear(LCD_COLOR_WHITE);
					if(strncmp(form, "CIRCLE", 6) == 0){
						BSP_LCD_DrawCircle( BSP_LCD_GetXSize() - x, y, z);
						CustomOut = 0;
					}
				}
			}
